...
# Ensure that the annotation data only contains samples present in decon_results$beta
sample_names <- colnames(decon_results$beta)
matching_indices <- annot_data$SegmentDisplayName %in% sample_names
annot_data_filtered <- annot_data[matching_indices, ]

# Now, extract U6ATAC_RNAscopeClass for the filtered annotation data
u6atac_class_filtered <- as.factor(annot_data_filtered$U6ATAC_RNAscopeClass)

# Create an annotation data frame with matching samples
annotation_data <- data.frame(U6ATAC_RNAscopeClass = u6atac_class_filtered)
rownames(annotation_data) <- annot_data_filtered$SegmentDisplayName
...
sorted_columns <- order(u6atac_class_filtered)
...
pdf("heatmap_sorted_U6ATAC_TME_class.pdf", width = 20, height = 12)
pheatmap(sorted_beta,
                 labels_row = rownames(decon_results$beta)[sorted_rows],  # Sorted row labels
                 main = "TME Deconvolution Heatmap by U6ATAC Class",
                 fontsize_row = 10,
                 fontsize_col = 10,
                 cluster_cols = FALSE,
                 cluster_rows = FALSE,
